> **[POLARIZED/폐기]** 이 문서의 §0·§9.1 "EAR과 대등 / 우열 판정 불가",
> §2 "디코더를 배포하지 않으므로 복원 수단 자체가 부재 → 원본 복원 불가",
> §12 로드맵, §13 "엣지 미확정" 은 **틀렸음.**
> 방법·모델 구조 서술(§3~§8)은 `docs/PROJECT.md` §8로 흡수되었다. **`docs/PROJECT.md` 참조.**
...
- **현재 상태 (2026-07-29)**:
  ...
  - **다음** — mEBAL2 도착 완료(2026-07-31). **180명이 아니라 58명.** 이후 전 실험 재실행.
