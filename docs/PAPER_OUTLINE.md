...
⚠️ 헤드라인 비교는 **두 추정량이 갈린다** — 풀링 CI 상한은 0 아래(−0.0020)지만 런평균
차이(−0.0019)가 fold 내 시드 std(0.0031)보다 작다. PROTOCOL §2 상 **"미측정"** 으로 적고
어느 쪽으로도 단정하지 않는다. 확실한 것은 **δ=0.02 기준 non_inferior** 뿐이다.
...
governor `performance`, `refine_landmarks=False`, **스로틀 없음**, 온도 47.7~50.5 °C)

| Method | crop | encode | decide | **method 소계** | e2e p50 | e2e p99 | FPS | frames |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| EAR (rule) | 0.08 | — | 0.14 | **0.22** | 9.56 | **10.10** | **104.2** | 31,268 |
| **Ours** | 0.67 | **0.86** | 0.25 | **1.78** | 11.27 | **11.85** | **88.5** | 26,543 |
| Image-CNN (max) | 0.67 | 2.04 | 0.17 | **2.88** | 12.26 | 12.87 | 81.3 | 24,398 |
| Image-CNN (head) | 0.67 | **2.05** | 0.31 | **3.03** | 12.51 | 13.10 | 79.7 | 23,909 |
...
⬜ **RSS peak · CPU% · 온도 추이**는 런 JSON 에 있으나 아직 표에 옮기지 않았다.
