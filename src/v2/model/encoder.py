"""v2 인코더 — 세로 해상도를 지키는 비대칭 CNN + 링버퍼 위의 양방향 시간 헤드.
...
~~**비대칭 stride 가 이 설계의 핵심이다.**~~ ← 아래 실측으로 **철회한다**.

> 🔴 **2026-08-06 T3-8 종결 — 위 1번 논거는 지지되지 않는다. 논문 Method 의 근거로
> 쓰지 말 것.**
>
> 교락 없는 대조군 `vdrop` 으로 검정했다. `vdrop` 은 vpres 와 커널·채널·가로 stride 가
> 전부 같고 **세로 stride 만** 16 이다(병목 눈꺼풀 0.55 px).
>
> | 비교 | 차이 (풀링, 짝지은 피험자 부트스트랩) |
> |---|---|
> | **vpres − vdrop** ★교락 없음 | **+0.0010 [-0.0032, +0.0069]** |
> | vpres − sym16 (교락됨) | +0.0002 [-0.0042, +0.0042] |
> | vpres − vfull | +0.0015 [-0.0016, +0.0040] |
>
> (`results/v2/cmp_vpres_vs_{vdrop,sym16,vfull}.json`, fold 0·1 × 3seed **탐색값**)
>
> "세로 stride 16 이면 신호가 **사라진다**"는 파국적 열화를 예측하지만 그런 일은
> 일어나지 않았다. 세로 해상도와 성능은 단조도 아니다
> (0.55 px → 0.9817·0.9832, 4.37 → 0.9850, 8.73 → 0.9825).
> **강한 형태는 반증됐고**, 0.005 미만의 약한 효과는 이 표본으로 구분할 수 없다
> ("기각"이 아니라 PROTOCOL §2 의 **"미측정"**).
>
> **그럼에도 vpres 를 유지한다** (2026-08-06 결정): 다운스트림 산출물이 전부 vpres
> 위에 있고, encode 가 e2e 의 4% 라 vdrop 의 3.6배 연산 이득이 e2e 로는 ~3% 밖에
> 안 된다. **전환 조건은 Pi 실측에서 G-E1(p99 ≤ 33.3 ms)을 못 맞출 때 하나뿐이다.**

**3. 시퀀스는 미래를 기다리지 않는(non-lookahead) 스트리밍으로 처리한다.**
...
> 프레임을 기다리지 않는다. 정확한 서술은 *"19프레임 링버퍼 위의 양방향 TCN. 미래
> 프레임을 기다리지 않으므로 시스템 추가 지연이 없다"* 이다.
> 논문에 "causal TCN" 은 틀린 말이다.
...
def temporal_head_mmac(d: int = 16, t: int = 19, layers: int = 3, k: int = 3) -> float:
    ...
    **프레임당이 아니라 호출당** 비용이다.
    ...
    ⚠️ 하네스는 **stride 1**(매 프레임 판정)로 돌리므로 프레임당 비용이 곧 이 값이다
    (0.0459 MMAC/frame). 창 경계에서만(stride 19) 판정하면 1/19 로 줄지만 깜빡임을
    최대 18프레임 늦게 잡는다 — 그래서 보고 기준은 stride 1 이다.
    """
    tcn = t * d * d * k * layers
